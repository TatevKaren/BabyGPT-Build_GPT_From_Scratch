...
d_k = 8 # number of features per head (should be smaller than d_model) or number of heads
Nx = 3 # number of blocks/number of layers
num_iter = 10000
...
class BigramLM(nn.Module):
    ...
    def __init__(self):
        ...
        # Repeating Multi Head Self Attention in sequential manner (Nx = 2 Blocks )
        #self.blocks = nn.Sequential(
        #    Block(d_model, h = 4),
        #    Block(d_model, h = 4),
        #    Block(d_model, h = 4),
        #) 
        self.blocks = nn.Sequential(*[Block(d_model, h) for _ in range(Nx)]) 
    ...
    def forward(self, idx, targets=None):    
        ...
        # one head self attention and feedforward in blocks
        X = self.blocks(X) 
        # getting the logits
